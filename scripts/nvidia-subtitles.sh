...
if [ -f "$map/$srt" ] && [[ $(find "$map/$srt" -type f -size +200c 2>/dev/null) ]] ; then
ffmpeg -hwaccel nvdec -i "$1" -i "$map/$srt" -c:v hevc_nvenc -preset:v hp -level:v 5.0 -rc:v vbr_hq -rc-lookahead:v 32 -brand mp42 -ac 2 -c:a aac -b:a 192k -c:s mov_text "$map/$mp4"
else echo "*** CCextractor couldn't find Closed Captions. No Subtitles will be added...***"
